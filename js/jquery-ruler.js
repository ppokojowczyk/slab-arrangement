(function($) {
  $.fn.ruler = function(options) {
     ...
     * in - inch
     */
    this._units = ['px', 'mm', 'cm', 'in' , 'artstonex'];
    ...
    this._updateRulerTicks = function(reset) {
      ...
      while (this._lastTopRulerPos < topRulerWidth) {
        if (this.options.tickMajor > 0 && (unitPos % this.options.tickMajor) === 0) {
          ...
          if (this.options.showLabel === true)
            $tick.text(unitPos * this.options.unitPosMultiplier);
      ...
      while (this._lastLeftRulerPos < leftRulerHeight) {
        if (this.options.tickMajor > 0 && (unitPos % this.options.tickMajor) === 0) {
          ...
          if (this.options.showLabel === true)
            $tick.append('<span>' + ( unitPos * this.options.unitPosMultiplier ) + '</span>')
    ...
    this.refresh = function() {
      switch (this.options.unit) {
        case 'artstonex':
          ...
          var __containerWidth = $('#formMainContainerWidth').val() * 1000;
          var __canvasWidth = $('#Canvas').width();
          var factor = ( __canvasWidth - 0 ) / __containerWidth;
          this._unitDiv = factor * 10;
          break;
        case 'px':
          this._unitDiv = 1;
          break;
